...
@implementation mgSearchThirdView_iPad
...
- (void)initMethod:(NSString *)qOrd qCd:(NSString *)qCd domCd:(NSString *)domCd type:(NSString *)type tecCd:(NSString *)tecCd title:(NSString *)title{
    ...
    if([type isEqualToString:@"1"] == YES){
        url = [NSString stringWithFormat:@"%@%@?acc_key=%@&domCd=%@&pageType=%@&qCd=%@&qOrd=%@&os=ios", URL_DOMAIN, URL_SEARCH_CHR_LIST, [_app._account getAcc_key], domCd, type, qCd, qOrd];
        
    //선생님별
    }else{
        url = [NSString stringWithFormat:@"%@%@?acc_key=%@&domCd=%@&pageType=%@&qCd=%@&qOrd=%@&qTecCd=%@&os=ios", URL_DOMAIN, URL_SEARCH_TEC_LIST, [_app._account getAcc_key], domCd, type, qCd, qOrd, tecCd];
    }
    
    detailWebView.delegate = self;
    [detailWebView loadRequest:[NSURLRequest requestWithURL:[NSURL URLWithString:url]]];
    [detailWebView setBackgroundColor:[UIColor clearColor]];
...
//    [CDNMoviePlayer getDeviceID];
    ...
    player = [[CDNMoviePlayer alloc] initWithContentURL:contentURL parentView:_app.navigationController bookmarkParam:0];
	[player setDelegate:self];
    
	[player allowsAirPlay:NO];
    
    NSString *nibName = @"PlayerControlDownViewController_iPad";
    
	controlViewController = [[PlayerControlDownViewController_iPad alloc] initWithNibName:nibName bundle:[NSBundle mainBundle]];
