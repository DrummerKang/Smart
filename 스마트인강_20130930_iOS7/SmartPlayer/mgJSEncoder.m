...
+ (NSString*) unescapeUnicodeString:(NSString*)string
{
    ...
    // 특수문자변환 (사파리에서 일부문자가 안보여서 그 부분은 특수처리
    // ・
    //KATAKANA MIDDLE DOT
    //유니코드: U+30FB, UTF-8: E3 83 BB
    //[tokenizedString replaceOccurrencesOfString:@"%B7" withString:@"%2F" options:NSLiteralSearch range:NSMakeRange(0, [tokenizedString length])];
    [tokenizedString replaceOccurrencesOfString:@"%B7" withString:@"%E3%83%BB" options:NSLiteralSearch range:NSMakeRange(0, [tokenizedString length])];
    // done
