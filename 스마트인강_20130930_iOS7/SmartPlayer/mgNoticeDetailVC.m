...
@interface mgNoticeDetailVC ()
@end

@implementation mgNoticeDetailVC

@synthesize gidx, _web, noticeTitle;
...
- (void)viewDidLoad{
    ...
    if([mgCommon osVersion_7]){
        ...
        if ([mgCommon hasFourInchDisplay]) {
            _web.frame = CGRectMake(0, 44, 320, 458 + 48);
        }else{
            _web.frame = CGRectMake(0, 44, 320, 416);
        }
    }
}
...
- (void)loadURL
{
    NSString *addr = [[NSString alloc]initWithFormat:@"%@%@?gidx=%@", URL_DOMAIN, URL_NOTICE_DETAIL, gidx];
