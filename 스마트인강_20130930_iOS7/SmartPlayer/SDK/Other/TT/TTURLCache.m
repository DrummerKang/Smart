...
#define MY_IMAGE_PATH @"http://image.popconn.com/me"
#define CHAT_IMAGE_PATH @"http://image.popconn.com/chat"
#define CHAT_THUMBNAIL @"thumbnail"
#define CHAT_ORIGINAL @"original"

#define IMAGE_SIZE 70.0f

//static const  CGFloat   kLargeImageSize = 600 * 400;
static const  CGFloat   kLargeImageSize = 1000 * 1000;

static        NSString* kDefaultCacheName       = @"Mill";
...
- (void)storeImage:(UIImage*)image forURL:(NSString*)URL force:(BOOL)force {
  if (nil != image && (force || !_disableImageCache)) {
	...
	/*
	  ...
	  if(isRemote == YES)	// http로 가져오는 경우 원하는 사이즈로 저장한다.
	  {
		 
		CGSize newSize = CGSizeMake(IMAGE_SIZE,IMAGE_SIZE);
		UIGraphicsBeginImageContext( newSize );
		[image drawInRect:CGRectMake(0,0,newSize.width,newSize.height)];
		UIImage* newImage = UIGraphicsGetImageFromCurrentImageContext();
		UIGraphicsEndImageContext();		
		[_imageCache setObject:newImage forKey:URL];
	  }
		else
			*/
