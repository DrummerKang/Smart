...
@implementation mgFirstProfileVC{
    UIToolbar *_tbKeyboard;
}

@synthesize firstNavigationBar;
@synthesize nickName;
@synthesize myGoal;
@synthesize userId;
...
- (void)viewDidLoad{
    ...
    nickName.inputAccessoryView = _tbKeyboard;
    myGoal.inputAccessoryView = _tbKeyboard;
    
    userId.text = [_app._account getUserID];
    
    [self initNavigationBar];
    
    bgImage = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"bg_white"]];
    bgImage.frame = CGRectMake(0, 44, 320, 400);
...
- (void)touchClose{
    ...
    [myGoal resignFirstResponder];
}

- (void)initNavigationBar{
    // 네비게이션바 배경 바꾸기
    [firstNavigationBar setBackgroundImage:[[UIImage imageNamed:@"_bg_navigator"]resizableImageWithCapInsets:UIEdgeInsetsMake(5, 5, 5, 5)] forBarMetrics:UIBarMetricsDefault];
    [firstNavigationBar setTitleTextAttributes:[NSDictionary dictionaryWithObjectsAndKeys:[UIColor colorWithRed:255.0/255.0 green:255.0/255.0 blue:255.0/255.0 alpha:1.0],UITextAttributeTextColor,
...
- (BOOL)textViewShouldBeginEditing:(UITextView *)textView{
    ...
    if([mgCommon osVersion_7]){
        if ([mgCommon hasFourInchDisplay]) {
            [textView setFrame:CGRectMake(0.0f, 60.0f, 320.0f, 262.0f)];
            
        }else{
            [textView setFrame:CGRectMake(0.0f, 60.0f, 320.0f, 180.0f)];
        }
        
    }else{
        if ([mgCommon hasFourInchDisplay]) {
            [textView setFrame:CGRectMake(0.0f, 44.0f, 320.0f, 256.0f)];
            
        }else{
            [textView setFrame:CGRectMake(0.0f, 44.0f, 320.0f, 166.0f)];
        }
    }
...
- (BOOL)textFieldShouldBeginEditing:(UITextField *)textField{
    CGRect keyboardRect = textField.bounds;
    keyboardRect = [self.view convertRect:keyboardRect fromView:nil];
    ...
    _rtGoal = textField.frame;
    
    // 7.0이상
    if([mgCommon osVersion_7]){
        if ([mgCommon hasFourInchDisplay]) {
            [textField setFrame:CGRectMake(0.0f, 20.0f, 320.0f, 256.0f)];
            
        }else{
            [textField setFrame:CGRectMake(0.0f, 20.0f, 320.0f, 160.0f)];
        }
        
    }else{
        if ([mgCommon hasFourInchDisplay]) {
            [textField setFrame:CGRectMake(0.0f, 0.0f, 320.0f, 256.0f)];
            
        }else{
            [textField setFrame:CGRectMake(0.0f, 0.0f, 320.0f, 160.0f)];
        }
    }
